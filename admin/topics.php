...
if (isset($_REQUEST['id']) && $_REQUEST['id'] != '') {
    ...
    if ($action == 'active') {
        $status = 1;
        $conn -> query("UPDATE `topics` SET `status` = '$status' WHERE `id`='$id'") or die("Updation Failed !!!.");
    } else if($action == 'deactive') {
        $status = 0;
        $conn -> query("UPDATE `topics` SET `status` = '$status' WHERE `id`='$id'") or die("Updation Failed !!!.");
    } else {
        $conn -> query("DELETE FROM `topics` WHERE `id` = '$id'") or die("Deletion Failed !!!.");
    }
}
...
<div class="main-content-header">
    <h2>Topics</h2>
    <a href="add-topic.php">&raquo; Add New Topic</a>
</div>
<div class="main">
    <table>
        <thead>
        ...
        <tbody>
            ...
            <tr>
                ...
                <?php 
                    ...
                    ?>
                    <a href="add-topic.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-primary" title="Edit"><i class="fa fa-edit"></i></a>
